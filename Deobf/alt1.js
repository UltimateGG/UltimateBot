...
/*function requestall() {
  ...
  var req = http.request(options, function(res) {
    ...
    if (kvalid == true) {
      ...
      var botUsername = process.env.alt1username;
      console.log('\x1b[37m[Alt1] '+botUsername+' launched!');
      ...
      function createBot() {
        bot = new mineflayer.createBot({
          ...
          username: process.env.alt1email,
          password: process.env.alt1password,
      ...
      //check sendchat.txt for any new data every second -with alt1 in it, if any, send it
      setInterval(function() {
        fs.readFile(assetsPath+'sendchat.txt', 'utf8', function(err, contents) {
          if (contents == '') {
          } else if (contents.includes('alt1')) {
            contents = contents.replace('alt1', '');
      ...
      function bindEvents(bot) {
        ...
        bot.on('end', function(reason) {
          ...
          fs.writeFile(assetsPath+'stats.txt', newStat, function (err) {if (err) {console.log(err);}});
          var online = false;
          console.log('[Alt1] Disconnected for some reason at ' + getTime() + ' reconnecting in 60 seconds.');
