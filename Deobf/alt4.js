  ...
  var req = http.request(options, function(res) {
    ...
    res.on('end', function () {
      try {
        ...
        return console.log('Error parsing verification text. This should never happen. Please try again and if it persists go to ULTIMATEBOT.PW and check if the website is up. If not open a ticket in discord.\n\n'+output);
      }
    ...
    if (kvalid == true) {
      ...
      var botUsername = process.env.alt4username;
      console.log('\x1b[37m[Alt4] '+botUsername+' launched!');
      ...
      function createBot() {
        bot = new mineflayer.createBot({
          ...
          username: process.env.alt4email,
          password: process.env.alt4password,
      ...
      //check sendchat.txt for any new data every second -with alt4 in it, if any, send it
      setInterval(function() {
        fs.readFile(assetsPath+'sendchat.txt', 'utf8', function(err, contents) {
          if (contents == '') {
          } else if (contents.includes('alt4')) {
            contents = contents.replace('alt4', '');
      ...
      function bindEvents(bot) {
        ...
        bot.on('end', function(reason) {
          ...
          fs.writeFile(assetsPath+'stats.txt', newStat, function (err) {if (err) {console.log(err);}});
          var online = false;
          console.log('[Alt4] Disconnected for some reason at ' + getTime() + ' reconnecting in 60 seconds.');
