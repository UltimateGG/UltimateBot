...
/*function requestall() {
  ...
  var req = http.request(options, function(res) {
    ...
    if (kvalid == true) {
      ...
      var botUsername = process.env.alt2username;
      console.log('\x1b[37m[Alt2] '+botUsername+' launched!');
      ...
      function createBot() {
        bot = new mineflayer.createBot({
          ...
          username: process.env.alt2email,
          password: process.env.alt2password,
      ...
      //check sendchat.txt for any new data every second -with alt2 in it, if any, send it
      setInterval(function() {
        fs.readFile(assetsPath+'sendchat.txt', 'utf8', function(err, contents) {
          if (contents == '') {
          } else if (contents.includes('alt2')) {
            contents = contents.replace('alt2', '');
      ...
      function bindEvents(bot) {
        ...
        bot.on('end', function(reason) {
          ...
          fs.writeFile(assetsPath+'stats.txt', newStat, function (err) {if (err) {console.log(err);}});
          var online = false;
          console.log('[Alt2] Disconnected for some reason at ' + getTime() + ' reconnecting in 60 seconds.');
